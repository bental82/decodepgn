...
which of **75 classic strategic “rules of thumb”** are relevant right then —
...
- **Quiz** — multiple-choice questions generated from this game and the 75 rules,
...
- **The 75 rules** — the full, searchable reference, with an **Ask** box on each
  rule.
...
3. That function calls Claude with the 75 rules and a **forced tool call** so the
...
The 75-rule prompt is identical on every request, so it’s sent as a **cached
...
src/
  ...
  shared/             rules.ts (the 75 rules) + request/response types (browser + server)
