...
which of **60 classic strategic “rules of thumb”** are relevant right then —
...
- **Quiz** — multiple-choice questions generated from this game and the 60 rules,
...
- **The 60 rules** — the full, searchable reference, with an **Ask** box on each
  rule.
...
3. That function calls Claude with the 60 rules and a **forced tool call** so the
...
src/
  ...
  shared/             rules.ts (the 60 rules) + request/response types (browser + server)
