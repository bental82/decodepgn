...
which of **96 classic strategic “rules of thumb”** are relevant right then —
...
- **Quiz** — two kinds. A **rules quiz**: multiple-choice questions generated
  from this game and the 96 rules. And a **best-move quiz**: positions from
...
- **The 96 rules** — the full, searchable reference, with an **Ask** box on each
  rule.
...
3. The app POSTs the game (for context), the target position, and the engine
   check to `/api/analyze`. That function calls Claude with the 96 rules and a
...
The 96-rule prompt is identical on every request, so it’s sent as a **cached
...
src/
  ...
  shared/             rules.ts (the 96 rules) + request/response types (browser + server)
